...
def post_get(fa):
	...
	r = requests.post('https://bio.vbio.top/api/bio/shrna-design/design/', 
		json=data)

	uuid = json.loads(r.text)['uuid']

	res = requests.get(f"https://bio.vbio.top/api/bio/shrna-design/result/{uuid}/")
	res = json.loads(res.text)
	return res['output']['result']
	# for sub_shRNA in res['output']['result']:
...
def blast(fa, trans_id, blast, blastdb):
	import subprocess
	blastn = subprocess.Popen(f"{blast} -db {blastdb} -task blastn-short -outfmt 6", 
		shell=True, 
...
def main(fa: str, output: str, blast: str, blastdb: str):
	...
	with open(fa) as fi, open(output,'w') as fo:
		fo.write('\t'.join(['score', 'shRNA_seq','startsite','id', 'num_of_hit', 'num_of_hit_self']) + '\n')
		for line in fi:
			if line.startswith('>'):
				cid = line.strip()[1:]
			else:
		...
		for label, seq_ in fa_pool.items():
			...
			for sub_shRNA in res:
				...
				fo.write('\t'.join([str(score), seq_, str(st), label, str(hit_n), str(hit_self)]) + '\n')
